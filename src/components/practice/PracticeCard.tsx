...
export function PracticeCard({ session }: PracticeCardProps) {
  return (
    <Link href={`/ensaios/${session.id}`}>
      <div className="practice-card bg-surface-raised rounded-xl p-4 border border-ink/10 shadow-sm hover:shadow-md hover:border-brand/40 transition-all cursor-pointer">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <Music className="w-4 h-4 text-brand" />
              <h3 className="font-semibold text-ink">{session.musicas.titulo}</h3>
            </div>
            <p className="text-sm text-ink-muted mb-3">{session.musicas.artista}</p>
